...
export function ModelAccessPicker({
  ...
  return <div className="grid gap-4">
    {categories.map((category) => {
      ...
      return <section className="rounded-xl border border-white/[0.08] bg-black/20 p-4" key={category.key}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h4 className="text-sm font-black text-white">{category.label}</h4><p className="mt-1 text-xs text-white/40">{category.description}</p></div>
          <div className="flex gap-2"><button className="text-xs font-bold text-[var(--brand-accent)] disabled:opacity-40" disabled={disabled} onClick={() => setCategory(category.key, options.map((item) => item.key))} type="button">Select all</button><button className="text-xs font-bold text-white/45 disabled:opacity-40" disabled={disabled} onClick={() => setCategory(category.key, [])} type="button">Clear</button></div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          {options.map((option) => <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-white/[0.07] bg-white/[0.025] p-3 text-xs text-white/65" key={option.key}><input checked={selected.has(option.key)} className="mt-0.5 accent-[var(--brand-primary)]" disabled={disabled} onChange={(event) => setCategory(category.key, event.target.checked ? [...selected, option.key] : [...selected].filter((key) => key !== option.key))} type="checkbox" /><span><strong className="block text-white/80">{option.model}</strong><span className="mt-1 block capitalize text-white/35">{option.provider}{option.kind === "realtime" ? " · realtime" : ""}</span></span></label>)}
          {!options.length ? <p className="text-xs text-amber-200/70">No configured models are available in this category.</p> : null}
        </div>
      </section>;
    })}
  </div>;
