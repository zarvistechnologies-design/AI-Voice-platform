...
function WorkflowDemo({ business, experience }: { business: BusinessUseCase; experience: UseCaseExperience }) {
  ...
  return (
    <div className="use-case-visual relative mx-auto w-full max-w-[500px] sm:px-4">
      <div className="relative z-10 overflow-hidden rounded-2xl border border-white/10 bg-black shadow-[0_28px_90px_rgba(0,0,0,0.55)]">
        <div className="flex items-center justify-between gap-4 border-b border-white/[0.08] px-5 py-3.5">
          <div className="flex items-center gap-3">
            <span className="grid size-9 place-items-center rounded-lg border border-[#35fbe0]/25 bg-[#35fbe0]/10 text-[11px] font-black text-[#75fff0]">
        ...
        <div className="px-5 pb-5 pt-3">
          ...
          <div className="mt-3 grid gap-3">
            <div className="mr-6 rounded-lg rounded-tl-sm border border-white/[0.08] bg-white/[0.04] p-3.5 sm:mr-10">
          ...
          <div className="mt-4 flex items-center gap-3 rounded-lg border border-[#35fbe0]/20 bg-[#35fbe0]/[0.06] p-3.5">
            <span className="grid size-7 shrink-0 place-items-center rounded-md bg-[#35fbe0] text-sm font-black text-[#031310]">&#10003;</span>
            <div>
  ...
  return (
    <SiteLayout>
      <div id="business-use-case-page" className="use-case-page overflow-hidden bg-black text-white">
            ...
            <div className="max-w-3xl">
              ...
              <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-400">
                {business.highlights.map((highlight) => (
                  <span className="inline-flex items-center gap-2" key={highlight}><span aria-hidden="true" className="text-base leading-none">&bull;</span>{highlight}</span>
                ))}
              </div>
            </div>
        ...
        <section className="bg-[#030605] px-5 py-12 sm:px-8 sm:py-14">
          <div className="mx-auto grid max-w-[1240px] gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12">
            <div className="lg:sticky lg:top-32 lg:self-start">
              <Pill>How it works</Pill>
              <h2 className="mt-5 text-2xl font-semibold leading-tight tracking-[-0.03em] sm:text-4xl">A clear path from workflow to live calls.</h2>
              <p className="mt-4 text-sm leading-6 text-slate-400 sm:text-base">Start with one high-value conversation, define its boundaries, and improve it from real outcomes.</p>
            </div>
          ...
          <div className="mx-auto max-w-[1100px] text-center">
            ...
            <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              {experience.integrations.map((integration, index) => (
                <div className="use-case-integration rounded-xl border border-white/[0.09] bg-black p-4 text-left transition" key={integration}><div className="flex items-center justify-between"><span className={`text-[10px] font-black ${index % 3 === 1 ? "text-[#c5bdff]" : index % 3 === 2 ? "text-[#ffb17e]" : "text-[#75fff0]"}`}>0{index + 1}</span><span className="size-2 rounded-sm bg-white/20" /></div><strong className="business-integration-marquee-name use-case-integration-name mt-7 block text-sm font-semibold">{integration}</strong></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="px-5 pb-14 sm:px-8 sm:pb-16">
          <div className="use-case-cta relative mx-auto flex max-w-[1040px] flex-col items-center justify-between gap-6 overflow-hidden rounded-2xl border border-[#35fbe0]/25 p-6 text-center sm:p-8 md:flex-row md:text-left lg:p-9">
            <div className="relative max-w-xl"><p className="text-[11px] font-black uppercase tracking-[0.14em] text-[#75fff0]">Ready to get started?</p><h2 className="mt-3 text-2xl font-semibold leading-tight tracking-[-0.03em] sm:text-3xl">Put {business.title.toLowerCase()} to work on one real call workflow.</h2></div>
            <Link className="relative inline-flex min-h-11 shrink-0 items-center rounded-lg bg-[#35fbe0] px-6 text-sm font-extrabold text-[#031310] transition hover:-translate-y-0.5 hover:bg-[#75fff0]" href="/contact">Contact us <span className="ml-3">&rarr;</span></Link>
          </div>
        </section>
      </div>
