...
function Code({ block }: { block: Extract<DocsBlock, { type: "code" }> }) {
  ...
  return <div className="docs-code-block overflow-hidden rounded-xl border border-white/10 bg-[#06100e]"><div className="flex justify-between border-b border-white/10 px-4 py-2 text-xs text-white/40"><span>{block.language}</span><button className="font-semibold text-[#75fff0]" onClick={() => void copy()} type="button">{copied ? "Copied" : "Copy"}</button></div><pre className="overflow-x-auto p-4 text-[13px] leading-6 text-[#c8faf2]"><code>{block.body}</code></pre></div>;
}
...
export function DocsArticle({ topic }: { topic: DocsTopic }) {
  ...
  return <div className="docs-page min-h-screen bg-[#020706] pt-24 text-white">
