...
type SessionRecord = { _id: string; device: string; ip: string; lastSeenAt: string; expiresAt: string; current: boolean };
...
export function ProfileShell() {
  ...
  const load = useCallback(async () => {
    try {
      setSessions((await accountApi.sessions()).sessions);
  ...
  async function changePassword(event: FormEvent) {
    ...
    await action(() => accountApi.changePassword(currentPassword, password), "Password changed and other sessions revoked.");
    setCurrentPassword("");
    setPassword("");
  }
  ...
  return (
    <main className="grid min-h-screen bg-[#f4f7fb] text-slate-950 lg:grid-cols-[64px_minmax(0,1fr)]">
      <DashboardSidebar activeLabel="Profile" userInitials={initials(session.name)} onLogout={() => void logoutSession().then(() => router.replace("/login"))} />
      <section className="min-w-0 p-4 sm:p-6 lg:p-8"><div className="mx-auto grid max-w-5xl gap-6">
        ...
        {notice ? <div className="break-words rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">{notice}</div> : null}
