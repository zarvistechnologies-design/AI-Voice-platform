...
function statusTone(status: AgentSummary["status"]) {
  ...
  return "border-[#dfe7e4] bg-[#f7f9f8] text-[#60716c]";
}
...
export function AgentsListShell() {
  ...
  if (!session) {
    return <main className="grid min-h-screen place-items-center bg-[#f7f9f8] text-sm font-semibold text-[#71817d]" role="status">Loading agents</main>;
  }
    ...
    <main className={`agents-home-palette grid min-h-screen w-full min-w-0 overflow-x-hidden bg-[#f7f9f8] text-[#14231f] ${
      ...
      <section className="grid min-w-0 content-start">
        <DashboardPageHeader
          ...
          actions={
            <button
              className="app-button-text inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg border border-[#118778] bg-[#118778] px-4 text-white shadow-[0_8px_18px_rgba(17,135,120,0.20)] transition hover:border-[#0e6f62] hover:bg-[#0e6f62] active:translate-y-px disabled:opacity-50 sm:w-auto"
              type="button"
              disabled={busy}
              onClick={() => {
          ...
          <div className="rounded-xl border border-[#dfe7e4] bg-white shadow-[0_10px_30px_rgba(34,38,74,0.05)]">
            <div className="flex flex-col gap-3 border-b border-[#e6e7ef] p-3 lg:flex-row lg:items-center">
              <div className="flex min-h-10 min-w-0 flex-1 items-center gap-2 rounded-lg border border-[#dfe7e4] bg-white px-3 transition focus-within:border-[#118778] focus-within:ring-4 focus-within:ring-[#118778]/10">
                ...
                <input
                  className="app-control-text min-h-9 min-w-0 flex-1 border-0 bg-transparent p-0 text-[#14231f] outline-none"
                  aria-label="Search agents"
                  value={query}
                  placeholder="Search by agent, team, or phone number"
                  onChange={(event) => setQuery(event.target.value)}
                />
                {query ? <button className="text-xs font-semibold text-[#118778] hover:text-[#0e6f62]" type="button" onClick={() => setQuery("")}>Clear</button> : null}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <label className="relative min-w-[132px] flex-1 lg:flex-none">
                  ...
                  <select
                    className="app-control-text min-h-10 w-full appearance-none rounded-lg border border-[#dfe7e4] bg-white py-0 pr-9 pl-3 text-sm font-semibold text-[#3b3e4e] outline-none transition hover:border-[#9fcfc3]"
                    value={statusFilter}
            ...
            {!loading && filteredAgents.map((agent) => (
              ...
              >
                ...
                >
                  <span className="grid size-10 place-items-center rounded-lg bg-[#edf7f4] text-[#0e6f62] ring-1 ring-[#118778]/10 transition group-hover:bg-[#e6e8fa]">
                    <Icon icon="agent" />
                  </span>
                  <span className="min-w-0">
                    <strong className="block truncate text-sm font-semibold text-[#20212b] transition group-hover:text-[#123d35] sm:text-[15px]">{agent.name}</strong>
                    <span className="mt-0.5 block truncate text-xs text-[#71817d]">{agent.team || "Voice team"}<span className="md:hidden"> · {agent.phone || "No phone assigned"}</span></span>
                  </span>
                  <span className="hidden min-w-0 items-center gap-2 text-sm text-[#52645f] md:flex">
                ...
                <span className="relative shrink-0">
                  <button
                    className="grid size-9 place-items-center rounded-lg border border-transparent text-[#71817d] transition hover:border-[#dfe7e4] hover:bg-white hover:text-[#123d35] active:translate-y-px disabled:opacity-50"
                    type="button"
                  ...
                  {openMenuId === agent._id ? (
                    <span className="absolute top-10 right-0 z-40 grid w-44 overflow-hidden rounded-xl border border-[#dfe7e4] bg-white p-1.5 shadow-[0_18px_45px_rgba(34,38,74,0.16)]" role="menu" onPointerDown={(event) => event.stopPropagation()}>
                      <button className="flex min-h-9 items-center gap-2 rounded-lg px-3 text-left text-sm font-medium text-[#3b3e4e] hover:bg-[#eef5f3] hover:text-[#123d35]" role="menuitem" type="button" onClick={() => beginEditAgent(agent)}><Icon icon="edit" />Edit name</button>
                      <button className="flex min-h-9 items-center gap-2 rounded-lg px-3 text-left text-sm font-medium text-[#3b3e4e] hover:bg-[#eef5f3] hover:text-[#123d35]" role="menuitem" type="button" onClick={() => void cloneAgent(agent)}><Icon icon="clone" />Clone agent</button>
            ...
            {!loading && !filteredAgents.length ? (
              <div className="grid min-h-56 place-items-center p-8 text-center">
                <div>
                  <span className="mx-auto grid size-12 place-items-center rounded-xl bg-[#edf7f4] text-[#0e6f62]"><Icon icon="agent" /></span>
                  <strong className="mt-4 block text-base font-semibold text-[#20212b]">No agents found</strong>
                  <span className="mt-1 block text-sm text-[#71817d]">Try another search or status filter.</span>
                  {(query || statusFilter !== "All") ? <button className="mt-4 text-sm font-semibold text-[#0e6f62] hover:text-[#123d35]" type="button" onClick={() => { setQuery(""); setStatusFilter("All"); }}>Clear filters</button> : null}
                </div>
              </div>
            ) : null}
            </div>
      ...
      {editingAgent ? (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#14231f]/35 px-4 py-4 backdrop-blur-[6px]" role="dialog" aria-modal="true" aria-labelledby="edit-agent-title" aria-describedby="edit-agent-description" aria-busy={busy}>
          <form
            className="grid max-h-[calc(100dvh-2rem)] w-full max-w-md gap-5 overflow-y-auto rounded-2xl border border-[#dfe7e4] bg-white p-6 shadow-[0_28px_80px_rgba(34,38,74,0.24)]"
            onSubmit={(event) => {
              event.preventDefault();
          ...
          >
            <div>
              ...
              <p className="mt-1 mb-0 text-sm text-[#71817d]" id="edit-agent-description">Update the name shown throughout this workspace.</p>
            </div>
            {notice ? <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700" role="alert">{notice}</div> : null}
            <label className="grid gap-1.5">
              <span className="text-xs font-semibold text-[#52645f]">Agent name</span>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                className="app-button-text inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[#118778] bg-[#118778] px-4 text-white shadow-sm transition hover:border-[#0e6f62] hover:bg-[#0e6f62] active:translate-y-px disabled:opacity-50"
                type="submit"
                disabled={busy}
              >
                <Icon icon="edit" />
