...
function roleTone(role: OrganizationRole) {
  ...
  if (role === "admin") return "bg-indigo-50 text-[#108D82] ring-indigo-200";
      ...
      <section className="min-w-0">
        <header className="border-b border-[#e5edf2] bg-white px-4 py-5 sm:px-6 lg:px-8">
          <div className="mx-auto w-full max-w-[1280px]">
            <span className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#108D82]">Workspace administration</span>
            <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
              ...
              <Link className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-bold text-slate-700 shadow-sm transition hover:border-[#b9ddd8] hover:text-[#108D82]" href="/dashboard/profile">
          ...
          <section className="relative overflow-hidden rounded-[26px] border border-[#e5e7ef] bg-white p-5 text-[#242535] shadow-sm sm:p-7">
            ...
            <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_230px] lg:items-center">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {activeOrganization ? (
                        <span className="rounded-full bg-[#108D82]/15 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wider text-[#108D82] ring-1 ring-[#108D82]/30">
              ...
              <div className="rounded-2xl border border-[#e5e7ef] bg-[#f6f7fb] p-5 backdrop-blur-sm">
                <span className="grid size-10 place-items-center rounded-xl bg-[#108D82]/15 text-[#108D82] ring-1 ring-[#108D82]/25"><WorkspaceIcon name="shield" /></span>
          ...
          <section className="grid min-w-0 gap-6 xl:grid-cols-[286px_minmax(0,1fr)]">
            <aside className="grid min-w-0 content-start gap-4 xl:sticky xl:top-6 xl:self-start">
                  ...
                  {loading && !organizations.length ? [0, 1].map((item) => <div className="h-[66px] animate-pulse rounded-xl bg-slate-100" key={item} />) : organizations.map((organization) => {
                    ...
                    return (
                      <button aria-current={active ? "true" : undefined} className={`grid w-full grid-cols-[40px_minmax(0,1fr)_auto] items-center gap-3 rounded-xl border p-2.5 text-left transition ${active ? "border-[#b9e5df] bg-[#edf9f7]" : "border-transparent hover:border-slate-200 hover:bg-slate-50"}`} disabled={busy} key={organization._id} onClick={() => void switchWorkspace(organization._id)} type="button">
                        <span className={`grid size-10 place-items-center rounded-xl text-xs font-black ${active ? "bg-[#108D82] text-[#108D82]" : "bg-slate-100 text-slate-700"}`}>{initials(organization.name)}</span>
                        ...
                        {active ? <span className="grid size-6 place-items-center rounded-full bg-[#108D82] text-[#ffffff]"><WorkspaceIcon className="size-3.5" name="check" /></span> : null}
                      </button>
                    );
                  })}
              ...
              <form className="grid gap-3 rounded-2xl border border-[#e2eaf0] bg-white p-4 shadow-sm" onSubmit={createWorkspace}>
                <span className="grid size-9 place-items-center rounded-xl bg-[#eaf9f7] text-[#108D82] ring-1 ring-[#cdeae6]"><WorkspaceIcon className="size-4.5" name="plus" /></span>
                ...
                <input autoComplete="organization" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm text-slate-950 outline-none transition focus:border-[#108D82] focus:ring-3 focus:ring-[#108D82]/20" id="new-workspace-name" minLength={2} placeholder="Workspace name" required value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} />
                <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#108D82] px-4 text-sm font-bold text-[#ffffff] transition hover:bg-[#108D82] disabled:cursor-not-allowed disabled:opacity-50" disabled={busy || workspaceName.trim().length < 2} type="submit"><WorkspaceIcon className="size-4" name="plus" />{activeAction === "create-workspace" ? "Creating..." : "Create workspace"}</button>
              </form>
              ...
              <nav aria-label="Workspace settings sections" className="flex gap-1 overflow-x-auto rounded-2xl border border-[#e2eaf0] bg-white p-2.5 shadow-sm xl:grid">
                {visibleSections.map((item) => (
                  <a className="inline-flex min-w-max items-center gap-2.5 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-600 transition hover:bg-[#edf9f7] hover:text-[#108D82]" href={`#${item.id}`} key={item.id}><WorkspaceIcon className="size-4" name={item.icon} />{item.label}</a>
                ))}
              </nav>
            </aside>
                ...
                {canManage ? (
                  <form className="grid gap-5 p-5 sm:p-6" onSubmit={saveWorkspace}>
                    <div className="grid gap-4 md:grid-cols-2">
                      <label className="grid gap-2 text-xs font-bold text-slate-700" htmlFor="organization-name">Organization name<input autoComplete="organization" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-normal text-slate-950 outline-none transition focus:border-[#108D82] focus:ring-3 focus:ring-[#108D82]/20" id="organization-name" minLength={2} required value={organizationName} onChange={(event) => setOrganizationName(event.target.value)} /></label>
                      <label className="grid gap-2 text-xs font-bold text-slate-700" htmlFor="organization-timezone">Timezone<input className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-normal text-slate-950 outline-none transition focus:border-[#108D82] focus:ring-3 focus:ring-[#108D82]/20" id="organization-timezone" placeholder="Asia/Kolkata" required value={organizationTimezone} onChange={(event) => setOrganizationTimezone(event.target.value)} /></label>
                      <label className="grid gap-2 text-xs font-bold text-slate-700" htmlFor="retention-days">Data retention<input className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-normal text-slate-950 outline-none transition focus:border-[#108D82] focus:ring-3 focus:ring-[#108D82]/20" id="retention-days" min={1} required type="number" value={dataRetentionDays} onChange={(event) => setDataRetentionDays(event.target.value)} /><span className="font-normal leading-5 text-slate-500">Number of days workspace data is retained.</span></label>
                      <div className="rounded-xl border border-[#dce9e6] bg-[#f7fbfa] p-4">
                        <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#108D82]">Workspace identity</span>
                    ...
                    <div className="flex flex-col justify-between gap-3 rounded-xl border border-[#e7eef2] bg-[#f8fafb] p-4 sm:flex-row sm:items-center">
                      ...
                      <button className="min-h-11 shrink-0 rounded-xl bg-[#108D82] px-5 text-sm font-extrabold text-[#ffffff] transition hover:bg-[#108D82] disabled:cursor-not-allowed disabled:opacity-50" disabled={busy || organizationName.trim().length < 2} type="submit">{activeAction === "save-workspace" ? "Saving..." : "Save settings"}</button>
                    </div>
                  </form>
                ) : (
              ...
              <article className="scroll-mt-6 overflow-hidden rounded-2xl border border-[#e2eaf0] bg-white shadow-sm" id="team">
                ...
                {canManage ? (
                  <form className="m-5 grid gap-4 rounded-2xl border border-[#dce9e6] bg-[#f7fbfa] p-4 sm:m-6 sm:p-5 lg:grid-cols-[minmax(0,1fr)_170px_auto] lg:items-end" onSubmit={inviteMember}>
                    <div className="lg:col-span-3"><span className="grid size-9 place-items-center rounded-xl bg-[#dff5f1] text-[#108D82]"><WorkspaceIcon className="size-4.5" name="invite" /></span><h3 className="mt-3 text-sm font-bold text-slate-950">Invite a teammate</h3><p className="mt-1 text-xs leading-5 text-slate-500">They will receive a secure email link to join this workspace.</p></div>
                    <label className="grid gap-2 text-xs font-bold text-slate-700" htmlFor="invite-email">Email address<input autoComplete="email" className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-normal text-slate-950 outline-none transition focus:border-[#108D82] focus:ring-3 focus:ring-[#108D82]/20" id="invite-email" placeholder="teammate@company.com" required type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} /></label>
                    <label className="grid gap-2 text-xs font-bold text-slate-700" htmlFor="invite-role">Workspace role<select className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold capitalize text-slate-950 outline-none focus:border-[#108D82]" id="invite-role" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as typeof inviteRole)}>{assignableRoles.map((role) => <option key={role} value={role}>{role}</option>)}</select></label>
                    <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#108D82] px-5 text-sm font-extrabold text-[#ffffff] transition hover:bg-[#108D82] disabled:cursor-not-allowed disabled:opacity-50" disabled={busy || !inviteEmail.trim()} type="submit"><WorkspaceIcon className="size-4" name="invite" />{activeAction === "invite-member" ? "Sending..." : "Send invite"}</button>
                    ...
                    return (
                      <div className="grid min-w-0 gap-4 border-b border-[#edf2f5] p-5 last:border-b-0 sm:grid-cols-[46px_minmax(0,1fr)_170px_auto] sm:items-center sm:px-6" key={member._id}>
                        ...
                        {canManage && member.role !== "owner" ? (
                          <label className="grid gap-1.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Role<select aria-label={`Role for ${member.userId.name}`} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold capitalize text-slate-800 outline-none focus:border-[#108D82]" disabled={busy} value={member.role} onChange={(event) => void runAction(roleAction, () => organizationApi.updateMember(member._id, event.target.value as Exclude<OrganizationRole, "owner">), "Member role updated.")}>{assignableRoles.map((role) => <option key={role} value={role}>{role}</option>)}</select></label>
