...
export function TestCallPanel({ agentId, agentName, knowledgeCount, recordingEnabled, onClose, onRegionChange }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid overflow-hidden overscroll-none bg-black/80 p-3 backdrop-blur-md sm:place-items-center sm:p-4">
      <section
        className="test-call-theme my-auto min-h-0 max-h-[calc(100dvh-1.5rem)] w-full min-w-0 max-w-lg overflow-y-auto overscroll-contain rounded-2xl border border-white/10 bg-[#ffffff] text-white shadow-[0_30px_100px_rgba(0,0,0,0.58)] [scrollbar-gutter:stable] sm:max-h-[calc(100dvh-2rem)]"
        role="dialog"
        aria-modal="true"
        ...
        aria-busy={busy}
      >
        <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-white/10 bg-[radial-gradient(circle_at_12%_0%,rgba(100,104,255,0.13),transparent_42%),#ffffff]/95 p-4 backdrop-blur-xl">
          <div className="min-w-0">
            <span className="text-[9px] font-extrabold uppercase tracking-[0.18em] text-[#454bd3]/55">Realtime voice session</span>
        ...
        <div className="grid min-w-0 gap-4 p-4 sm:gap-5 sm:p-5">
          <div className="grid grid-cols-2 gap-1 rounded-xl border border-white/10 bg-[#ffffff] p-1" role="group" aria-label="Test call type">
            {(["web", "phone"] as const).map((item) => (
              <button
                className={`app-button-text flex-1 rounded-lg px-3 py-2 transition active:translate-y-px disabled:opacity-50 ${mode === item ? "bg-[#6468ff] text-[#ffffff] shadow-[0_6px_18px_rgba(100,104,255,0.16)]" : "text-white/56 hover:bg-white/[0.05] hover:text-white/78"}`}
                key={item}
                type="button"
          ...
          <div className="relative grid min-h-52 min-w-0 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_70%_0%,rgba(100,104,255,0.20),transparent_46%),linear-gradient(145deg,#000_0%,#ffffff_62%,#ffffff_100%)] p-4 text-white shadow-inner sm:p-5">
            <span className="pointer-events-none absolute -top-16 -right-10 size-44 rounded-full bg-[#6468ff]/10 blur-2xl" />
            <span className="pointer-events-none absolute -bottom-20 -left-10 size-48 rounded-full bg-[#6468ff]/[0.07] blur-2xl" />
            <div className="relative grid min-w-0 max-w-full place-items-center gap-4 text-center">
              <div className={`relative grid size-20 place-items-center rounded-full border border-[#6468ff]/25 bg-[#6468ff]/10 text-[#454bd3] ${active ? "ring-8 ring-[#6468ff]/[0.07]" : ""}`}>
                {active ? <span className="absolute inset-0 animate-ping rounded-full bg-[#6468ff]/15 motion-reduce:animate-none" /> : null}
              ...
              <div className="flex h-8 items-center gap-1" role="img" aria-label={active ? "Call audio active" : "Call audio inactive"}>
                {[16, 28, 20, 32, 24, 30, 18].map((height, index) => (
                  <span
                    className={`w-1.5 rounded-full bg-[#454bd3] ${active ? "animate-pulse motion-reduce:animate-none" : "opacity-30"}`}
                    key={`${height}-${index}`}
          ...
          {mode === "phone" && !active ? (
            <label className="grid gap-2 text-xs font-bold text-white/56">
              ...
              <input
                className="app-control-text min-h-11 rounded-xl border border-white/10 bg-[#ffffff] px-3 text-white outline-none transition placeholder:text-white/32 focus:border-[#6468ff] focus:ring-3 focus:ring-[#6468ff]/15"
          ...
          <button
            className={`app-button-text min-h-12 rounded-xl px-4 font-extrabold transition active:translate-y-px disabled:cursor-not-allowed disabled:opacity-50 ${active ? "border border-rose-400/25 bg-rose-400/15 text-rose-200 hover:bg-rose-400/20" : "bg-[#6468ff] text-[#ffffff] shadow-[0_12px_30px_rgba(100,104,255,0.18)] hover:bg-[#5057e5]"}`}
            type="button"
            disabled={busy}
            aria-busy={busy}
            aria-describedby="test-call-status"
