              ...
              return (
                <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" key={provider.id}>
                  <div className="p-5">
                    ...
                    <div className="mt-5 flex gap-2">{provider.id === "vobiz" ? <Link className="rounded-xl bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-white" href="/dashboard/phone-number" prefetch={false} onFocus={() => router.prefetch("/dashboard/phone-number")} onMouseEnter={() => router.prefetch("/dashboard/phone-number")} onPointerDown={() => router.prefetch("/dashboard/phone-number")}>Manage Vobiz</Link> : provider.connected ? <button className="rounded-xl border border-rose-200 px-4 py-2.5 text-sm font-semibold text-rose-700" disabled={busy} type="button" onClick={() => void disconnect(provider.id as Exclude<IntegrationProvider["id"], "vobiz">)}>Disconnect</button> : <button className="rounded-xl bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-white" type="button" onClick={() => { setSelected(provider.id as Exclude<IntegrationProvider["id"], "vobiz">); setCredential(""); }}>Connect {item.name}</button>}</div>
                  </div>
                </article>
              );
            })}
