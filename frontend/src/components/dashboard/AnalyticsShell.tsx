...
const palette = ["#108D82", "#108D82", "#a855f7", "#f59e0b", "#ef5d7a"];
  ...
  return (
    <div className="relative mt-5">
      ...
      <svg className="h-72 w-full overflow-visible" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Interactive call volume and completed calls trend" onPointerLeave={() => setHovered(null)} onPointerMove={(event) => { const bounds = event.currentTarget.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (event.clientX - bounds.left) / bounds.width)); setHovered(Math.round(ratio * (data.length - 1))); }}>
        <defs><linearGradient id="trend-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#108D82" stopOpacity=".34"/><stop offset="1" stopColor="#108D82" stopOpacity="0"/></linearGradient></defs>
        ...
        <polyline points={points} fill="none" stroke="#108D82" strokeWidth="1.7" vectorEffect="non-scaling-stroke" />
        <polyline points={completePoints} fill="none" stroke="#108D82" strokeWidth="1.4" strokeDasharray="4 3" vectorEffect="non-scaling-stroke" />
        {hovered !== null ? <><line x1={(hovered / Math.max(1, data.length - 1)) * 100} x2={(hovered / Math.max(1, data.length - 1)) * 100} y1="12" y2="100" stroke="rgba(255,255,255,.35)" strokeWidth=".5" strokeDasharray="2 2"/><circle cx={(hovered / Math.max(1, data.length - 1)) * 100} cy={92 - (data[hovered].calls / max) * 72} r="1.4" fill="#ffffff" stroke="#108D82" strokeWidth=".8" vectorEffect="non-scaling-stroke"/></> : null}
    ...
    <main className={`grid min-h-screen bg-[#f7f8fc] text-[#242535] ${showUserSidebar ? "lg:grid-cols-[272px_minmax(0,1fr)]" : "lg:grid-cols-[64px_minmax(0,1fr)]"}`}>
      ...
      <section className="min-w-0 overflow-hidden bg-white p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-[1540px]">
          ...
          <section className="hidden">
            <article className="rounded-2xl border border-[#108D82]/20 bg-white p-5 sm:p-6"><span className="text-[10px] font-bold uppercase tracking-[.2em] text-[#108D82]">{insight.tone}</span><h2 className="mt-3 text-xl font-semibold leading-7">{insight.headline}</h2><p className="mt-3 text-sm leading-6 text-[#737587]">{insight.body}</p><button className="mt-5 rounded-lg border border-[#108D82]/20 bg-[#108D82]/10 px-3 py-2 text-xs font-semibold text-[#108D82]" type="button" onClick={() => router.push("/dashboard/calls")}>Review relevant calls →</button></article>
          ...
          <section className="hidden">
            <article className="rounded-2xl border border-[#e5e7ef] bg-[#ffffff]/85 p-5 sm:p-6"><div className="flex items-start justify-between"><div><h2 className="text-base font-semibold">Best time to reach customers</h2><p className="mt-1 text-xs text-[#737587]">Call activity by hour of day · hover any cell</p></div><span className="rounded-lg bg-[#108D82]/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#108D82]">24-hour map</span></div><ActivityHeatmap rows={data.hourlyActivity} /></article>
          ...
          <section className="hidden">
            <article className="rounded-2xl border border-[#e5e7ef] bg-[#ffffff]/85 p-5 sm:p-6"><h2 className="text-base font-semibold">Customer sentiment</h2><p className="mt-1 text-xs text-[#737587]">Tone detected across analyzed conversations</p><BreakdownBars rows={data.sentimentBreakdown} total={data.sentimentBreakdown.reduce((sum, row) => sum + row.value, 0)} colors={["#22c98f", "#108D82", "#ef5d7a"]} /></article>
