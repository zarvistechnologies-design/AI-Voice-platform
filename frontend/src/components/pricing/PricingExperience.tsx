    ...
    <div className="pricing-page bg-white text-[#171719]">
        ...
        <div className="mx-auto flex max-w-[1320px] flex-col items-center text-center">
          ...
          <nav className="pricing-nav mt-7 flex flex-wrap justify-center gap-2" aria-label="Pricing page sections">
            {[
              ...
              <a className="rounded-full border border-white/10 bg-white/[0.035] px-4 py-2 text-xs font-semibold text-white/60 transition hover:border-[#6268ff]/30 hover:text-[#565dcc]" href={href} key={href}>{label}</a>
            ))}
          </nav>
        </div>
      </section>
      ...
      <section className="scroll-mt-20 px-4 py-14 sm:px-6 sm:py-16 lg:px-8" id="pricing-plan">
        <div className="mx-auto max-w-[1320px]">
          ...
          <article className="pricing-plan-card relative grid overflow-hidden rounded-3xl border border-[#6268ff]/15 bg-[#f7f6ff] shadow-[0_28px_80px_rgba(70,65,130,0.12)] lg:grid-cols-[0.85fr_1.15fr]">
            <span className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#6268ff]/80 to-transparent" aria-hidden="true" />
            <div className="border-b border-white/10 bg-[linear-gradient(145deg,rgba(98,104,255,0.09),transparent_52%)] p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-9">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#6268ff] px-5 text-sm font-bold text-white transition hover:bg-[#565dcc]" href="/login">Get started <ArrowIcon /></Link>
            ...
            <div className="p-6 sm:p-8 lg:p-9">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#565dcc]">Included with usage</p>
              ...
              <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                {includedServices.map((service) => (
                  <li className="flex min-h-12 items-center gap-3 rounded-xl border border-white/[0.07] bg-black/15 px-4 py-3 text-sm leading-5 text-white/65" key={service}>
                    <span className="grid size-5 shrink-0 place-items-center rounded-full bg-[#6268ff]/10 text-[10px] font-black text-[#565dcc]" aria-hidden="true">✓</span>
                    {service}
                  </li>
                ))}
              </ul>
      ...
      <section className="scroll-mt-20 bg-black px-4 py-14 sm:px-6 sm:py-16 lg:px-8" id="calculator">
        <div className="mx-auto max-w-[1320px]">
          ...
          <div className="pricing-calculator-card grid overflow-hidden rounded-3xl border border-white/10 bg-[#07100e] shadow-[0_28px_80px_rgba(0,0,0,0.28)] lg:grid-cols-[0.95fr_1.05fr]">
            <div className="border-b border-white/10 p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-9">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {calculatorPresets.map((preset) => {
                  ...
                  return (
                    <button
                      className={`min-h-10 rounded-xl border px-3 text-xs font-semibold transition ${isActive ? "border-[#6268ff]/60 bg-[#6268ff]/10 text-[#454bd0]" : "border-white/10 bg-black/20 text-white/55 hover:border-[#6268ff]/25 hover:text-[#565dcc]"}`}
                      key={preset.label}
                      type="button"
              ...
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                <label className="grid gap-2 text-sm font-semibold text-white/70">
                  Number of calls
                  <input className="h-12 rounded-xl border border-white/10 bg-black/50 px-4 text-white outline-none transition focus:border-[#6268ff]/60 focus:ring-2 focus:ring-[#6268ff]/10" min="0" max="1000000" inputMode="numeric" type="number" value={calls} onChange={(event) => setCalls(event.target.valueAsNumber || 0)} />
                </label>
                <label className="grid gap-2 text-sm font-semibold text-white/70">
                  Average minutes per call
                  <input className="h-12 rounded-xl border border-white/10 bg-black/50 px-4 text-white outline-none transition focus:border-[#6268ff]/60 focus:ring-2 focus:ring-[#6268ff]/10" min="0" max="240" step="0.5" inputMode="decimal" type="number" value={minutes} onChange={(event) => setMinutes(event.target.valueAsNumber || 0)} />
                </label>
              </div>
            </div>

            <div className="pricing-estimate-panel flex flex-col justify-between bg-[linear-gradient(145deg,rgba(98,104,255,0.10),rgba(255,255,255,1)_55%)] p-6 sm:p-8 lg:p-9">
              <div>
      ...
      <section className="scroll-mt-20 bg-black px-4 py-14 sm:px-6 sm:py-16 lg:px-8" id="faq">
        <div className="mx-auto grid max-w-[1320px] gap-10 lg:grid-cols-[0.55fr_1.45fr]">
          ...
          <div className="grid gap-3">
            {faqs.map((faq) => (
              <details className="pricing-faq-card group rounded-2xl border border-white/[0.08] bg-[#f7f6ff] px-5 transition open:border-[#6268ff]/20 open:bg-[#f2f0ff]" key={faq.question}>
                <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-5 py-3 text-[15px] font-semibold text-white marker:content-none">
                  {faq.question}
                  <span className="grid size-8 shrink-0 place-items-center rounded-full border border-white/10 bg-black/20 text-lg font-normal text-[#565dcc] group-open:hidden" aria-hidden="true">+</span>
                  <span className="hidden size-8 shrink-0 place-items-center rounded-full border border-[#6268ff]/20 bg-[#6268ff]/10 text-lg font-normal text-[#565dcc] group-open:grid" aria-hidden="true">−</span>
                </summary>
